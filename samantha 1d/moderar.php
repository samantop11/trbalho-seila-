...
if(isset($_POST['atualiza'])){
    ...
    $sql = "UPDATE samantha SET nome='$nome', email='$email', mensagem='$msg' WHERE id=$idatualiza";
...
if(isset($_GET['acao']) && $_GET['acao'] == 'excluir'){
    $id = intval($_GET['id']);
    mysqli_query($conexao, "DELETE FROM samantha WHERE id=$id") or die("Erro ao deletar: " . mysqli_error($conexao));
    header("Location: moderar.php");
    exit;
}
...
$recado_editar = null;
if($editar_id){
    $res = mysqli_query($conexao, "SELECT * FROM samantha WHERE id=$editar_id");
...
<form method="post">
    ...
    <input type="submit" name="atualiza" value="Modificar samantha" class="btn"/>
</form>
</div>
<?php endif; ?>

<?php
$seleciona = mysqli_query($conexao, "SELECT * FROM samantha ORDER BY id DESC");
if(mysqli_num_rows($seleciona) <= 0){
    echo "<p>Nenhum pedido no mural!</p>";
}else{
    while($res = mysqli_fetch_assoc($seleciona)){
        echo '<ul class="samantha">';
